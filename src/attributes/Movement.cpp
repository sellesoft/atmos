...
void Movement::DecideMovementState() {
	...
	if (phys->contactState == ContactNONE) { moveState = InAirNoInput; inAir = true; }
		...
		if (bonGround) {
			if (inputs != vec3::ZERO) {
				if (DeshInput->KeyDownAnyMod(Key::LCTRL))  moveState = OnGroundCrouching;
				else if (DeshInput->KeyDownAnyMod(Key::LSHIFT)) moveState = OnGroundRunning;
...
void Movement::GrabObject() {
	...
	persist int frame = DeshTime->updateCount;
	...
	if (DeshInput->KeyPressedAnyMod(DeshKeys.use) && DeshTime->updateCount != frame) {
		if (!grabbing) {
			//find obj to pick up
			//TODO(sushi, Cl) make this a function somewhere, maybe geometry, and make the editor and this call it 
			vec3 pos = Math::ScreenToWorld(DeshInput->mousePos, camera->projMat, camera->viewMat, DeshWindow->dimensions);
			...
			int  index = 0;
			bool done = false;
			for (Entity* e : DeshAdmin->entities) {
							...
							for (u32 i = 0; i < b.indexArray.size(); i += 3) {
								...
								if (normal.dot(p0 - camera->position) < 0) {
									...
									if (t > 0) {
										...
										vec3 perp0 = normal.cross(p1 - p0).yInvert().normalized();
										vec3 perp1 = normal.cross(p2 - p1).yInvert().normalized();
										vec3 perp2 = normal.cross(p0 - p2).yInvert().normalized();
			...
			if (closeindex != -1) {
				grabeephys = DeshAdmin->entities[closeindex]->GetComponent<Physics>();
	...
	if (grabeephys && grabbing) {
		...
		vec3 cenpos =
			Math::ScreenToWorld(DeshWindow->dimensions / 2, camera->projMat, camera->viewMat, DeshWindow->dimensions)
			+ camera->forward * 3;
		Math::clamp(timer, 0, timetocenter);
		if (timer < timetocenter) {
			timer += DeshTime->fixedDeltaTime;
...
void Movement::Update() {
	...
	vec3 standpos = DeshAdmin->player->transform.position + vec3::UP * 2;
	vec3 crouchpos = DeshAdmin->player->transform.position + vec3::UP * 0.5;
	persist vec3 cpos = standpos;
	persist float timer = 0;
	float ttc = 0.2;

	if (DeshInput->KeyDownAnyMod(DeshKeys.movementCrouch)) {
		if (timer < 0.2) timer += DeshTime->fixedDeltaTime;
	}
	else {
		if (timer > 0) timer -= DeshTime->fixedDeltaTime;
	}
	...
	//apply gravity
	phys->velocity += vec3(0, -9.81, 0) * DeshTime->fixedDeltaTime;
	...
	auto accel = [&](float max, float accel) {
		...
		//	"vel      ", phys->velocity, "\n",
		//	"input    ", inputs));
		//if (projvel < max - accel * DeshTime->fixedDeltaTime) 
		//	phys->velocity += DeshTime->fixedDeltaTime * accel * inputs;
		//else if (maxWalkingSpeed - fabs(DeshTime->fixedDeltaTime * accel) <= projvel && projvel < max)
		//	phys->velocity += (max - phys->velocity.mag() * cosf(Math::AngBetweenVectors(phys->velocity, inputs))) * inputs;
		phys->velocity += accel * DeshTime->fixedDeltaTime * inputs;
	...
	//
	//if (projVel < maxWalkingSpeed - DeshTime->deltaTime * gndAccel) {
	//	phys->velocity += DeshTime->fixedDeltaTime * gndAccel * inputs;
	//}
	//else if (maxWalkingSpeed - DeshTime->deltaTime * gndAccel <= projVel && projVel < maxWalkingSpeed){
		...
		if (phys->velocity != vec3::ZERO) {
			if (phys->velocity.mag() > 0.12) {
				for (auto& m : phys->manifolds) {
					...
					phys->velocity += phys->acceleration * DeshTime->fixedDeltaTime;
				}
			}
	...
	phys->position += phys->velocity * DeshTime->fixedDeltaTime;
